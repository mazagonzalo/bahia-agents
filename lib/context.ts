...
export async function getClubContext(opts?: {
  ...
  const [trendsRes, eventsRes, reviewsRes, memoryRes] = await Promise.all([
    ...
    supabase
      ...
      .select('*')
      .eq('active', true)
      .or(`type.eq.recurrente,and(type.eq.especial,start_date.gte.${fmt(today)},start_date.lte.${fmt(future)})`)
      .order('content_potential', { ascending: false })
      .limit(8),

    // Reseñas recientes (tabla puede no existir aún — falla silencioso)
    supabase
      .from('reviews')
      .select('*')
    ...
    // Memoria de agentes: aprendizajes recientes con buen outcome
    supabase
      .from('agent_memory')
      .select('*')
      .in('outcome', ['bueno', 'malo'])
      .gte('created_at', sevenDaysAgo.toISOString())
      .order('created_at', { ascending: false })
      .limit(10)
      .then(res => {
        if (opts?.agents?.length) {
          return { ...res, data: res.data?.filter(m => opts.agents!.includes(m.agent)) ?? [] }
        }
        return res
      }),
  ])
  ...
  return {
    ...
    upcomingEvents: (eventsRes.data ?? []) as ClubEvent[],
    reviews: (reviewsRes.data ?? []) as Review[],
    memory: (memoryRes.data ?? []) as AgentMemory[],
  }
}
