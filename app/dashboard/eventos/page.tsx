...
// hairline dorado y sombras difusas. Sin sombras duras ni tipografía genérica.
function PosterCard({ poster, innerRef }: { poster: Poster; innerRef: React.RefObject<HTMLDivElement | null> }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', inset: 20, border: '1px solid rgba(228,199,134,0.45)', pointerEvents: 'none' }} />
      ...
      <div style={{ position: 'relative', zIndex: 2, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: 46 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div style={{ fontFamily: SERIF, fontSize: 26, letterSpacing: 8, color: '#F3E7C9', fontWeight: 600, lineHeight: 1 }}>BAHÍA</div>
            <div style={{ fontSize: 8.5, letterSpacing: 4, color: 'rgba(228,199,134,0.75)', textTransform: 'uppercase', marginTop: 5 }}>Social Sports Club</div>
          </div>
          {poster.sport && (
            <span style={{ fontSize: 10, letterSpacing: 2.5, textTransform: 'uppercase', border: '1px solid rgba(228,199,134,0.55)', color: '#F3E7C9', padding: '6px 13px', borderRadius: 999, backdropFilter: 'blur(2px)' }}>{poster.sport}</span>
          )}
        </div>

        <div>
          {poster.subtitle && (
            <div style={{ fontSize: 11.5, letterSpacing: 3.5, color: '#E4C786', textTransform: 'uppercase', marginBottom: 14, fontWeight: 500 }}>{poster.subtitle}</div>
          )}
          ...
          <div style={{ height: 1.5, width: 72, background: 'linear-gradient(90deg, #E4C786, rgba(228,199,134,0.15))', margin: '22px 0 20px' }} />
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 26px', display: 'flex', flexDirection: 'column', gap: 9 }}>
              {poster.bullets.map((b, i) => (
                <li key={i} style={{ fontSize: 13.5, display: 'flex', gap: 11, alignItems: 'baseline', color: 'rgba(255,255,255,0.90)', letterSpacing: 0.1 }}>
                  <span style={{ display: 'inline-block', width: 5, height: 5, borderRadius: '50%', background: '#E4C786', flexShrink: 0, transform: 'translateY(-2px)' }} />{b}
                </li>
              ))}
            </ul>
          )}
          {poster.cta && (
            <div style={{ display: 'inline-block', background: 'linear-gradient(135deg, #E4C786, #C9A85C)', color: '#0A1024', fontWeight: 700, fontSize: 14, letterSpacing: 0.4, padding: '13px 28px', borderRadius: 999, boxShadow: '0 12px 34px -10px rgba(201,168,92,0.55)' }}>{poster.cta}</div>
          )}
        </div>
      </div>
    </div>
